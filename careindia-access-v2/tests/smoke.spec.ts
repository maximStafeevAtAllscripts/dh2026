...
test('smoke test - app loads care desert indicators page', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: 'CareAccess India' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Planning Priorities' })).toBeVisible();
  await expect(page.getByText('India · District Level')).toBeVisible();
  await expect(page.getByText('Risk Categories')).toBeVisible();
  await expect(page.getByText('District Priorities')).toBeVisible();
  await expect(page.getByText('Select a district from the map or list')).toBeVisible();

  await page.getByRole('button').filter({ hasText: 'Population impact' }).first().click();
